...
//                        keySet.forEach(item-> redisTemplate.delete(item));
//                    }
                    if (!StringUtils.isEmpty(s)) {
                        redissonClient.getKeys().deleteByPattern(s + "*");
                    }

                }
                // 取消直接删除， 直接删除集群环境会报：
                // org.springframework.dao.InvalidDataAccessApiUsageException: CROSSSLOT Keys in request don't hash to the same slot.
//                if (key.length == 1) {
...
//                redisTemplate.delete(keys);
    ...
    public void delByPattern(String patternKey) {
        if (!StringUtils.isEmpty(patternKey)) {
            redissonClient.getKeys().deleteByPattern(patternKey);
        }
    }
