...
//            FileDownloadUtils.downloadFile(fileName, Base64.decodeBase64(fileInfo), response);
            ...
            SysFileManage sysFileManage = sysFileManageService.getSysFileManageById(id);
            String fileName = sysFileManage.getFileName();
            if (!fileName.contains(sysFileManage.getFileType())) {
                fileName = sysFileManage.getFileName() + sysFileManage.getFileType();
            }
            FileDownloadUtils.downloadFile(fileName, sysFileManageService.downloadFileNew(sysFileManage,token), response);
