...
//        mergeData(returnLeave);
    ...
    public MapViewData getLeavingRatio(String year, String administrativeLevel, String areaCode, String dataType,
        ...
        Map<String, String> regionNameMapsByCodes = SysRegionUtil.getRegionNameMapByCodes(IdUtil.getIdsByStr(childrenRegionIds), regionYear);
        // Map<String, String> regionNameMapsByCodes = syncSysRegionService.getNameMap(IdUtil.getIdsByStr(childrenRegionIds), regionYear);
        if (!isSixRegion) {
            String yearByYear = SysRegionUtil.getYearByYear(year);
            // String yearByYear = syncSysRegionService.getYearByYear(year);
            // String level = syncSysRegionService.getLevel(areaCode, yearByYear);
            String level = Objects.requireNonNull(SysRegionUtil.getRegionCodeByLastCode2(areaCode, yearByYear)).getRegionLevel();
            Map<Integer, BigDecimal> returnRatioMap = new HashMap<>();
            ...
            for (String areaIdKey : regionNameMapsByCodes.keySet()) {
                ...
                if (RegionLevel.COUNTY.getCode().equals(level) && dataType.equals("1")) {
                    BigDecimal bigDecimalValue = returnRatioMap.get(Integer.parseInt(areaIdKey));
            ...
            if (StringUtils.isBlank(sixRegionCodes)) {
                ...
                Map<String, String> regionNameMapsByCodes1 = SysRegionUtil.getRegionNameMapByCodes(IdUtil.getIdsByStr(sixRegionCodes), regionYear);
    ...
    public List<ColumnPieChartVo> getLeavingPieChart(String year, String areaCode, String dataType, boolean isSixRegion, String sixRegionCodes) {
        ...
        String retionYear = SysRegionUtil.getYearByYear(year);
        ...
        if (CollectionUtils.isEmpty(areaCode1)) {
            ...
            String level = Objects.requireNonNull(SysRegionUtil.getRegionCodeByLastCode2(areaCode, retionYear)).getRegionLevel();
    ...
    private String getChildrenIds(String areaCode, String year) {
        ...
        String childrenRegionIds = SysRegionUtil.getChildrenRegionIdStrByYearList(areaCode, year);
        if (StringUtils.isBlank(childrenRegionIds)) {
            // 如果是县级就查询本身的数据
            String yearByYear = SysRegionUtil.getYearByYear(year);
            // String yearByYear = syncSysRegionService.getYearByYear(year);
