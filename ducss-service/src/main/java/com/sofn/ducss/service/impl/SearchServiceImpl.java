...
import com.sofn.ducss.enums.CropsSortEnum;
...
//                        HashMap<String, Map<String, Object>> map = new HashMap<>();
    ...
    private void changeErrorInfo(List<DataAnalysisProvice> list) {
        ...
        Result<Map<String, String>> result = sysApi.getRegionNameMapsByCodes(areaCodes, null);
        Map<String, String> stringMap = new HashMap<>();
        if (result.getData() != null) {
            stringMap = result.getData();
        }
        //秸秆类型
     ...
     */
    @Override
    public List<DataKing> getDataKingList(Map<String, String> paramMap) throws Exception {
        if (null != paramMap) {
            //单选作物类型（我是00000）：查出全部再分页
            if (paramMap.get("allCropType").contains("0")) {
                //总条数
