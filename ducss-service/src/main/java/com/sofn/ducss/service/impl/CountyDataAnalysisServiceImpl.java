...
public class CountyDataAnalysisServiceImpl extends ServiceImpl<CountyDataAnalysisMapper, DataAnalysisArea> implements CountyDataAnalysisService {
    ...
    @Override
    public List<DataAnalysisArea> listByYearAndAreaIdsAndTotolRateTooMuch(String year, List<String> areaIds) {
        return baseMapper.listByYearAndAreaIdsAndTotolRateTooMuch(year, areaIds);
    }
    ...
    public void insertCountyDataAnalysis(String year, String areaCode) {
        ...
        //获取省市县名字并拼接
        AreaRegionCode result = SysRegionUtil.getRegionCodeByLastCode(areaCode, year);
...
//            area.setId(IdUtil.getUUId());
            ...
            // 只用于计算综合利用率，不做其他展示
            area.setCollectResourceV2(area.getCollectResource());
            area.setStrawUtilizationV2(area.getStrawUtilization());
            list.add(area);
        }
        dataAnalysisAreaMapper.insertList(list);
    ...
    public void insertCityDataAnalysis(String year, String areaCode) {
        //查询当前区域下一级区域id
        List<String> areaList = SysRegionUtil.getChildrenRegionIdByYearList(areaCode, year);
        ...
        //获取省市县名字并拼接
        AreaRegionCode resultName = SysRegionUtil.getRegionCodeByLastCode(areaCode, year);
    ...
    public void insertProvinceDataAndSixRegionAnalysis(String year, String areaCode) {
        ...
        String regionName = SysRegionUtil.getRegionNameByRegionCode(areaCode, year);
        //获取秸秆类型map集合
        List<SysDict> dictList = SysDictUtil.getDictList(Constants.DictionaryType.STRAW_TYPE);
        HashMap<String, String> strawTypeMap = new HashMap<>(14);
