...
public class AggregateServiceImpl implements AggregateService {
     ...
     * @return
     */
    private StrawProduceResVo2 addSumProduce2(List<StrawProduceResVo2> resList) {
     ...
     * @return
     */
    private StrawProduceResVo2 addProduce2(StrawProduceResVo2 mapItem, StrawProduceResVo2 produce) {
        mapItem.setTheoryResource(mapItem.getTheoryResource().add(produce.getTheoryResource()));
        mapItem.setCollectResource(mapItem.getCollectResource().add(produce.getCollectResource()));
        //粮食产量sum
    ...
    private List<StrawProduce> getStrawProduceByRegion(AreaRegionCode regionCode, String year) {
        ...
        if (regionCode.getRegionLevel().equals(RegionLevel.COUNTY.getCode())) {  //查询县级数据
            ...
            String childrenIds = SysRegionUtil.getChildrenRegionIdStrByYearList(regionCode.getLastId(), year);
...
//        sum.setComprehensive(StrawCalculatorUtil.calculationComprehensiveUtilizeRatio(sum.getProStrawUtilize(),sum.getMainTotalOther(),sum.getYieldAllExport(),sum.getCollectResource()));
     ...
     * @return
     */
    private StrawUtilizeResVo3 addSumUtilize2(List<StrawUtilizeResVo3> resList) {
        StrawUtilizeResVo3 sum = new StrawUtilizeResVo3();

        // 计算综合利用率使用
        // BigDecimal totalStrawUsage = BigDecimal.ZERO;
        ...
        for (StrawUtilizeResVo3 utilize : resList) {
            ...
            /*
            if (utilize.getCollectResourceV2().compareTo(BigDecimal.ZERO) > 0
                    && utilize.getStrawUsageV2().compareTo(BigDecimal.ZERO) > 0) {
                totalStrawUsage = totalStrawUsage.add(utilize.getStrawUsageV2());
                totalCollectResource = totalCollectResource.add(utilize.getCollectResourceV2());
            }
            */
        }
        //计算综合利用率
    ...
    private List<StrawUtilizeResVo3> getStrawUtilizeDataByRegionList2(List<AreaRegionCode> regionList, String year, Boolean checkInfoFlag) {
        ...
        for (StrawUtilizeResVo3 utilize : utilizes) {
            ...
            if (mapItem != null) {
                ...
                /*
                if (utilize.getStrawUsageV2().compareTo(BigDecimal.ZERO) == 0
                        || utilize.getCollectResourceV2().compareTo(BigDecimal.ZERO) == 0) {
                    utilize.setStrawUsageV2(BigDecimal.ZERO);
                    utilize.setCollectResourceV2(BigDecimal.ZERO);
                }
                */
                utilizeMap.put(utilize.getStrawType(), utilize);
            }
    ...
    private StrawUtilizeResVo3 addStrawUtilize2(StrawUtilizeResVo3 utilizeSource, StrawUtilizeResVo3 a) {
        ...
        /*
        if (a.getStrawUsageV2().compareTo(BigDecimal.ZERO) > 0
                && a.getCollectResourceV2().compareTo(BigDecimal.ZERO) > 0) {
            utilizeSource.setStrawUsageV2(utilizeSource.getStrawUsageV2().add(a.getStrawUsageV2()));
            utilizeSource.setCollectResourceV2(utilizeSource.getCollectResourceV2().add(a.getCollectResourceV2()));
        }
        */
//        utilizeSource.setDisperseFertilising(utilizeSource.getDisperseFertilising().add(utilizeAdd.getDisperseFertilising()));
//        utilizeSource.setDisperseForage(utilizeSource.getDisperseForage().add(utilizeAdd.getDisperseForage()));
...
//        utilizeSource.setTheoryResource(utilizeSource.getTheoryResource().add(utilizeAdd.getTheoryResource()));
     ...
     * @return
     */
    private StrawUtilizeResVo3 addStrawUtilizeNoReturnSource(StrawUtilizeResVo3 utilizeSource, StrawUtilizeResVo3 a) {
    ...
    private List<StrawUtilizeResVo> getStrawUtilizeDataByRegion(AreaRegionCode region, String year) {
        ...
        if (region.getRegionLevel().equals(RegionLevel.COUNTY.getCode())) {  //查询县级
            ...
            String childrenIds = SysRegionUtil.getChildrenRegionIdStrByYearList(region.getLastId(), year);
    ...
    private List<StrawUtilizeResVo3> getStrawUtilizeDataByRegion2(AreaRegionCode region, String year, List<String> status) {
        ...
        if (region.getRegionLevel().equals(RegionLevel.COUNTY.getCode())) {  //查询县级
            ...
            if (sysOrganization.getOrganizationLevel().equals(RegionLevel.COUNTY.getCode())) {
                utilizes = countyAggregateService.getCountyStrawUtilize2(region.getLastId(), year);
            } else {
    ...
    private List<StrawUtilizeResVo3> assemblyStrawUtilizeBySusList2(List<StrawUsageSum> susList) {
        ...
        for (StrawUsageSum sus : susList) {
            ...
            BeanUtils.copyProperties(sus, temp);
            // temp.setStrawUsageV2(sus.getStrawUtilizationV2());
            resList.add(temp);
        }
        return resList;
    }
    ...
    public List<StrawProduceUtilizeResVo> findStrawProduceAndUtilzeData(AggregateQueryVo vo) {
        ...
        if (resList.size() > 0) {
            ...
            final Map<String, String> nameMap = SysRegionUtil.getRegionNameMapByCodes(areaIds, vo.getYear());
            resList.forEach(p -> {
    ...
    private List<StrawProduceUtilizeResVo> getStrawProduceUtilizeResVo(List<ProductionUsageSum> p) {
        ...
        if (p == null || p.size() == 0) {
            ...
            for (ProductionUsageSum pus : p) {
                ...
                s.setCollectResourceV2(pus.getCollectResourceV2());
                s.setProStrawUtilizeV2(pus.getStrawUtilizationV2());
...
//        pu.setIndustrializationIndex(StrawCalculatorUtil.calculationinIndustrializationIndex(pu.getMainTotal(),pu.getCollectResource()));
    ...
    private StrawProduceUtilizeResVo addProduceUtilize(StrawProduceUtilizeResVo pu, StrawProduceUtilizeResVo target) {
        ...
        if (target.getProStrawUtilizeV2().compareTo(BigDecimal.ZERO) > 0
                && target.getCollectResourceV2().compareTo(BigDecimal.ZERO) > 0) {
            pu.setProStrawUtilizeV2(pu.getProStrawUtilizeV2().add(target.getProStrawUtilizeV2()));
            pu.setCollectResourceV2(pu.getCollectResourceV2().add(target.getCollectResourceV2()));
        }
     ...
     * @throws com.sofn.common.exception.SofnException 如果有错误信息直接抛出
     */
    private List<AreaRegionCode> checkQueryRights(AggregateQueryVo vo) {
        ...
        for (String areaId : ids) {
            temp = SysRegionUtil.getRegionCodeByLastCode(areaId, vo.getYear());
            regionList.add(temp);
...
//        Set<String> provinceSet = new HashSet<>();  //省
//        Set<String> citySet = new HashSet<>();  //市
        temp = SysRegionUtil.getRegionCodeByLastCode(id, vo.getYear());
        regionList.add(temp);
        Result<List<SysRegionTreeVo>> listByParentId = sysApi.getListByParentId(id, Constants.APPID, "", null);
        List<SysRegionTreeVo> sysRegionTreeVoList = listByParentId.getData();
        for (SysRegionTreeVo sysRegionTreeVo : sysRegionTreeVoList) {
            temp = SysRegionUtil.getRegionCodeByLastCode2(sysRegionTreeVo.getRegionCode(), vo.getYear());
            if (temp != null) {
                regionList.add(temp);
            }
...
//            ServiceHelpUtil.throwException(errMsg);
     ...
     * @return
     */
    private ReturnLeaveSumVo addReturnLeaveList(List<ReturnLeaveSumVo> resList) {
     ...
     * @return
     */
    private ReturnLeaveSumVo addReturnLeave(ReturnLeaveSumVo mapItem, ReturnLeaveSumVo produce) {
...
//                PageHelper.offsetPage(vo.getPageNo(),vo.getPageSize());
                ...
                //查询区域名称
                final Map<String, String> regionNameMap = SysRegionUtil.getRegionNameMapByCodes(validIds, vo.getYear());
    ...
    private List<AreaRegionCode> getAuditPassedRegionCodeByParent(AreaRegionCode parentRegion, String year) {
        ...
        List<AreaRegionCode> children = SysRegionUtil.getChildrenRegionCodeList(parentRegion.getLastId(), year);
