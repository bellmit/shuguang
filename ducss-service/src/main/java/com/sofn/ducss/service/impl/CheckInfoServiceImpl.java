...
public class CheckInfoServiceImpl implements CheckInfoService {
    ...
    public List<CheckInfo> getCheckInfo(String year, String areaId, String status) {
        ...
        // 组装数据
        List<String> areaIds = SysRegionUtil.getChildrenRegionIdByYearList(areaId, year);
        ...
        Map<String, String> regionNameMap = SysRegionUtil.getRegionNameMapByCodes(areaIds, year);
        ...
        } else if (RegionLevel.PROVINCE.getCode().equals(organizationLevel)) {
            columnName = "city_id";
        ...
        if (CollectionUtils.isNotEmpty(checkInfoChina)) {
            long s2 = System.currentTimeMillis();
            log.info("查询areaId开始============" + s);
            List<String> areaId1 = syncSysRegionService.getAreaIdByStatus(year, Lists.newArrayList(areaId),
                    Lists.newArrayList("1", "5", "3", "2"));
            long s3 = System.currentTimeMillis() - s2;
            log.info("查询areaId结束============" + s3);
            if (CollectionUtils.isNotEmpty(areaId1)) {
    ...
    private CheckInfo getCountCheckInfo(List<CheckInfo> checkInfoChina,
        ...
        if (!CollectionUtils.isEmpty(checkInfoChina)) {
            ...
            for (CheckInfo item : checkInfoChina) {
                ...
                item.setSynUtilizeNumSum(item.getCollectNumSum().compareTo(BigDecimal.ZERO) == 0 ? BigDecimal.ZERO : item.getStrawUtilizeNumSum().divide(item.getCollectNumSum(), 4, RoundingMode.HALF_UP).multiply(new BigDecimal(100)));
                if (item.getCollectResourceV2().compareTo(BigDecimal.ZERO) > 0 && item.getStrawUtilizationV2().compareTo(BigDecimal.ZERO) > 0) {
                    strawUtilizationV2 = strawUtilizationV2.add(item.getStrawUtilizationV2());
                    collectResourceV2 = collectResourceV2.add(item.getCollectResourceV2());
                }
            }
