/**
 * 此类除开集成了原始的Excel生成的方法外，新增了一个response直接输出的口子，不再依赖于原来的生成本地文件后，再下载的方式
 * createExcel(Class cls, List<E> list, HttpServletResponse response,String fileName)
 *
 * @auth 文俊云
 */
...
public class ExportUtil {
     ...
     */
    public ExportUtil(Class<?> cls, ExcelField.Type type) {
        this(cls, type, null);
    }
     ...
     */
    public ExportUtil(Class<?> cls, ExcelField.Type type, String sheetNameOrIndex) {
    ...
    public void createSheet(String sheetName, String title, List<String> headerList, List<Integer> headerWidthList) {
        ...
        Row headerRow = sheet.createRow(StringUtils.isEmpty(title) ? rownum.get() : rownum.incrementAndGet());
    ...
    /**
     * 创建sheet，导出标题有部分合并（使用上的限制，合并的属性需要连续且不间断）
     *
     * @param sheetName       sheet名称
     * @param title           标题
     * @param headerList      列标题
     * @param headerWidthList 列宽度
     * @param margeMap  标题合并的名称
     */
    public void createSheet(String sheetName, String title, List<String> headerList, List<Integer> headerWidthList,Map<String,List<String>> margeMap) {
        if (margeMap.isEmpty()){
            ...
            Row margeRow =  sheet.createRow(StringUtils.isEmpty(title) ? rownum.get(): rownum.incrementAndGet());
            Row headerRow = sheet.createRow(rownum.incrementAndGet());
            headerRow.setHeightInPoints(16);
            for (int i = 0; i < headerList.size(); i++) {
                Cell cell = headerRow.createCell(i);
                Cell mergeCell = margeRow.createCell(i);
                cell.setCellStyle(styles.get("header"));
                mergeCell.setCellStyle(styles.get("header"));
                cell.setCellValue(headerList.get(i));
            }
            // 设置每列的宽度
            boolean isDefWidth = (headerWidthList != null && headerWidthList.size() == headerList.size());
            ...
            Map<String,List<Integer>> margeCellMap =  new HashMap<>();
            for (String key : margeMap.keySet()){
                List<String> margeCells = margeMap.get(key);
                List<Integer> numList = new ArrayList<>();
                for (int i =0;i<headerRow.getLastCellNum();i++){
                    Cell  heaqdCell= headerRow.getCell(i);
                    String cellValue = heaqdCell.getStringCellValue();
                    if (margeCells.contains(cellValue)){
                        numList.add(heaqdCell.getColumnIndex());
                    }
                }
                margeCellMap.put(key,numList);
            }
            for (String key : margeCellMap.keySet()){
                List<Integer> cellNumList = margeCellMap.get(key);
                Cell cell = margeRow.getCell(cellNumList.get(0));
                cell.setCellValue(key);
                CellRangeAddress region = new CellRangeAddress(margeRow.getRowNum(),margeRow.getRowNum(),cellNumList.get(0),cellNumList.get(cellNumList.size()-1));
                sheet.addMergedRegion(region);
            }
            //处理合并列的未合并项目
            for (int i=0;i<margeRow.getLastCellNum();i++){
                Cell cell = margeRow.getCell(i);
                List<CellRangeAddress> list = sheet.getMergedRegions();
                boolean flag = false;
                for (int j=0;j<list.size();j++){
                    if (list.get(j).isInRange(cell)){
                        flag = true;
                        break;
                    }
                }
                if (!flag){
                    ...
                    CellRangeAddress region = new CellRangeAddress(cell.getRowIndex(),cell.getRowIndex()+1,cell.getColumnIndex(),cell.getColumnIndex());
                    sheet.addMergedRegion(region);
                }
            }
        }
    ...
    public void createSheet(String sheetName, Class cls, ExcelField.Type type) {
        ...
        for (Object[] titleInfo : titleInfos) {
            ...
            String marge = (String)titleInfo[5];
            if (!StringUtils.isEmpty(marge)){
                List<String> margeList = mergeMap.get(marge);
                if (margeList == null){
                    margeList = new ArrayList<>();
                }
                margeList.add((String) titleInfo[0]);
                mergeMap.put(marge,margeList);
            }
        }
        if (mergeMap.isEmpty()){
            this.createSheet(sheetName, title, headerList, headerWidthList);
        }else {
            this.createSheet(sheetName, title, headerList, headerWidthList,mergeMap);
        }
    }
     ...
     * @return list 数据列表
     */
    public <E> ExportUtil setDataList(List<E> list, ExcelField.Type type) {
        if (list != null) {
            // 创建数据
            for (E e : list) {
                int colunm = 0;
    ...
    /**
     * 添加数据（通过annotation.ExportField添加数据）
     * update by wuXY 2019-11-22 17:37:30
     *
     * @return list 数据列表
     */
    public <E> ExportUtil setDataList(Class cls, List<E> list, ExcelField.Type type) {
        if (list != null) {
            // 创建数据
            for (E e : list) {
                int colunm = 0;
            ...
            // 创建模板
//            createTemplate(ImportUserBean.class, this.sheet, type);
            //update by wuXY 2019-11-22 17:32:31
            createTemplate(cls, this.sheet, type);
            log.info("模板创建成功");
     ...
     * 输出到文件
     */
    public ExportUtil writeFile(String name) throws IOException {
     ...
     * 输出到文件
     */
    public ExportUtil writeFile(HttpServletResponse response) throws IOException {
     ...
     * @param <E>      E
     */
    public static <E> void createExcel(Class cls, List<E> list, HttpServletResponse response, String fileName) {
        createExcel(cls, list, ExcelField.Type.EXPORT, response, fileName);
    }
    ...
    private static <E> void createExcel(String filePath, Class cls, List<E> list, ExcelField.Type type) {
        ExportUtil ee = null;
        if (type != null) {
            ee = new ExportUtil(cls, type);
        } else {
            ee = new ExportUtil(cls);
        }
     ...
     * @param <E>      E
     */
    private static <E> void createExcel(Class cls, List<E> list, ExcelField.Type type, HttpServletResponse response, String fileName) {
        ExportUtil ee = null;
        ...
        //update by wuXY 2019-11-22 17:38:55
        //ee.setDataList(list, type);
        ee.setDataList(cls, list, type);
        try {
            response.reset();
     ...
     * @param
     * @return
     * @description 创建多了sheet
     */
    public static <E> void createMoreExcel(List<Class> cls, List<List<E>> datas, List<String> sheetName, HttpServletResponse response, ExcelField.Type type, String fileName) {
        OutputStream os = null;
     ...
     *
     * @param cla
     * @param selectFields 要查询的字段集合
     * @return
     */
    public static Class<T> filterField(Class cla, String selectFields) {
...
/*        if (CollectionUtils.isEmpty(selectFields)) {
        ...
        String selectFieldStr = String.join(",", selectFields);*/
        String annotationNameStr = getAnnotationNameStr(cla);
        try {
            ExcelReflectionUtil.doChangeExcelFieldIsShowVaule(cla, selectFields, annotationNameStr);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return cla;
    ...
    public static String getAnnotationNameStr(Class cla) {
        List<String> annotationNames = getAnnotationNames(cla);
        return String.join(",", annotationNames);
    }

    /**
     * 获取该类中有 @ExcelField 注解的字段
     *
     * @param cla
     * @return
     */
    public static List<String> getAnnotationNames(Class cla) {
        List<String> annotationNames = Lists.newArrayList();
        Field[] fields = cla.getDeclaredFields();
        for (Field field : fields) {
            if (field.isAnnotationPresent(ExcelField.class)) {
                annotationNames.add(field.getName());
            }
        }
