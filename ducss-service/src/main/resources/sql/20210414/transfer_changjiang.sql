-- 2020-4-9
-- 469926 ： /海南省/昌江黎族自治县/昌江黎族自治县虚拟县
-- 469026 ： /海南省/昌江黎族自治县
-- 昌江没有2020年的数据 只处理18 19年的数据

-- 1.迁移产生量与直接还田量填报
UPDATE pro_still SET area_id= '469926',city_id = '469026'  WHERE  ID IN (SELECT ID
FROM pro_still WHERE area_id = '469026' );
-- 2.农户分散利用量填报
UPDATE disperse_utilize SET area_id = '469926',city_id = '469026'  WHERE ID IN (SELECT "id" FROM disperse_utilize WHERE area_id = '469026' );
UPDATE country_task SET area_id = '469926',city_id = '469026' WHERE id in (SELECT "id" FROM country_task WHERE area_id = '469026' );
-- 3.迁移市场主体利用量填报数据
UPDATE straw_utilize SET area_id = '469926',city_id = '469026' WHERE ID IN (SELECT "id" FROM straw_utilize  WHERE year = '2018' AND area_id = '469026');

-- 插入18 19年昌江虚拟县的数据 （之前处理数据的时候处理掉了）
...
FROM
	...
	AND A.YEAR IN('2018','2019') AND a.area_id = '469926'
	) C;
...
WHERE
	D.B_ID = A.B_ID AND A.YEAR IN('2018','2019') AND a.area_id = '469926';
...
update data_analysis_area set RETURN_PERS = return_ratio / 100 where YEAR IN('2018','2019') and area_id = '469926';
...
WHERE
	A.B_ID = D.B_ID AND A.YEAR IN('2018','2019') and A.area_id = '469926';
...
FROM
    ...
    WHERE
	A.B_ID = F.B_ID AND F.YEAR IN('2018','2019') AND F.AREA_ID = '469926';
...
FROM
    ...
    where t2.year IN('2018','2019') and t2.area_id = '469926'
	GROUP BY
		B_ID
	) D
WHERE
...
SET REUSE = FERTILISINGD + FORAGED + FUELD + BASED + MATERIALD WHERE YEAR IN('2018','2019') AND AREA_ID = '469926';
...
FROM
	...
	FROM
		DATA_ANALYSIS_AREA
	where
	    year in ('2018','2019') and AREA_ID = '469926'
	GROUP BY
		AREA_ID,
	YEAR
	) B
...
FROM
	DATA_ANALYSIS_AREA
WHERE YEAR IN('2018','2019') and area_id = '469926'
GROUP BY
...
CASE
	...
	END WHERE YEAR IN('2018','2019') and city_id = '469026';
...
FROM
	...
	FROM
		data_analysis_city
	WHERE
	    year in ('2018','2019') and city_id = '469026'
	GROUP BY
		city_ID,
	YEAR
	) B
...
-- 更新area表中的 area_id为昌江县的id
update data_analysis_area set area_id = '469926' where area_id = '469026';

-- 更新area汇总表中 区域名称
update data_analysis_area set area_name = '/海南省/昌江黎族自治县/昌江黎族自治县虚拟县' where area_id = '469926';

-- 更新city汇总表中 区域名称
update data_analysis_city set area_name = '/海南省/昌江黎族自治县' where city_id = '469026';

-- 更新city_id
UPDATE straw_utilize_sum set province_id = '460000',city_id = '469026' where area_id = '469026';

UPDATE straw_utilize_sum set city_id = '469026' where area_id = '469926';
...
from (
	SELECT * from straw_utilize_sum where year in ('2018','2019') and area_id = '469926'
) b
where
	a.area_id = b.area_id and a.year = b.year and a.straw_type = b.straw_type and a.area_id = '469926';
...
--return_pers = , -- 单作物的直接还田率
from (
	SELECT * from straw_utilize_sum where year in ('2018','2019') and city_id = area_id and province_id != city_id and area_id = '469026'
) b
where
...
update collect_flow set city_id = '469026' where area_id = '469926';
update collect_flow set city_id = '469026', level = '4' where area_id = '469026';
...
-- 上报管理
delete from country_task where id in('b482fea98efa494da3168f557d088338','76f0155a3e924f9f988550845ef28f9d','0d68dfc9853041ed993687d748df390e');
