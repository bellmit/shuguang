-- 2020-4-9
-- 469925 ： /海南省/白沙黎族自治县/白沙黎族自治县虚拟县
-- 469025 ： /海南省/白沙黎族自治县
-- 白沙没有2020年的数据


-- 1.迁移产生量与直接还田量填报
UPDATE pro_still SET area_id= '469925',city_id = '469025'  WHERE  ID IN (SELECT ID
FROM pro_still WHERE area_id = '469025' );
-- 2.农户分散利用量填报
UPDATE disperse_utilize SET area_id = '469925',city_id = '469025'  WHERE ID IN (SELECT "id" FROM disperse_utilize WHERE area_id = '469025' );
UPDATE country_task SET area_id = '469925',city_id = '469025' WHERE id in (SELECT "id" FROM country_task WHERE area_id = '469025' );
-- 3.迁移市场主体利用量填报数据
UPDATE straw_utilize SET area_id = '469925',city_id = '469025' WHERE ID IN (SELECT "id" FROM straw_utilize  WHERE year = '2018' AND area_id = '469025');

-- 插入18 19年白沙市虚拟县的数据 （之前处理数据的时候处理掉了）
...
FROM
	...
	AND A.YEAR IN('2018','2019') AND a.area_id = '469925'
	) C;
...
WHERE
	D.B_ID = A.B_ID AND A.YEAR IN('2018','2019') AND a.area_id = '469925';
...
update data_analysis_area set RETURN_PERS = return_ratio / 100 where YEAR IN('2018','2019') and area_id = '469925';
...
WHERE
	A.B_ID = D.B_ID AND A.YEAR IN('2018','2019') and A.area_id = '469925';
...
FROM
    ...
    WHERE
	A.B_ID = F.B_ID AND F.YEAR IN('2018','2019') AND F.AREA_ID = '469925';
...
FROM
    ...
    where t2.year IN('2018','2019') and t2.area_id = '469925'
	GROUP BY
		B_ID
	) D
WHERE
...
SET REUSE = FERTILISINGD + FORAGED + FUELD + BASED + MATERIALD WHERE YEAR IN('2018','2019') AND AREA_ID = '469925';
...
FROM
	...
	FROM
		DATA_ANALYSIS_AREA
	where
	    year in ('2018','2019') and AREA_ID = '469925'
	GROUP BY
		AREA_ID,
	YEAR
	) B
...
FROM
	DATA_ANALYSIS_AREA
WHERE YEAR IN('2018','2019') and area_id = '469925'
GROUP BY
...
CASE
	...
	END WHERE YEAR IN('2018','2019') and city_id = '469025';
...
FROM
	...
	FROM
		data_analysis_city
	WHERE
	    year in ('2018','2019') and city_id = '469025'
	GROUP BY
		city_ID,
	YEAR
	) B
...
-- 更新area表中的 area_id为白沙县的id
update data_analysis_area set area_id = '469925' where area_id = '469025';

-- 更新area汇总表中 区域名称
update data_analysis_area set area_name = '/海南省/白沙黎族自治县/白沙黎族自治县虚拟县' where area_id = '469925';

-- 更新city汇总表中 区域名称
update data_analysis_city set area_name = '/海南省/白沙黎族自治县' where city_id = '469025';

-- 更新city_id
UPDATE straw_utilize_sum set province_id = '460000',city_id = '469025' where area_id = '469925';
UPDATE straw_utilize_sum set city_id = '469025' where area_id = '469025';
...
--	export_yield_total,
--	grain_yield
--) SELECT (id::int4 + 1111) as "id",YEAR
--,
--province_id,
--city_id,
--'469924',
...
from (
	SELECT * from straw_utilize_sum where year in ('2018','2019') and area_id = '469925'
) b
where
	a.area_id = b.area_id and a.year = b.year and a.straw_type = b.straw_type and a.area_id = '469925';
...
--return_pers = , -- 单作物的直接还田率
from (
	SELECT * from straw_utilize_sum where year in ('2018','2019') and city_id = area_id and province_id != city_id and area_id = '469025'
) b
where
...
update collect_flow set city_id = '469025' where area_id = '469925';
update collect_flow set city_id = '469025', level = '4' where area_id = '469025';
...
-- 上报管理
delete from country_task where id in('cd7760efdb294f19bc130fe721ea073c','c35688f092ca4399bb8109f42913a0ad','4400a3a7ee07448abe766a8093dcf63c');
update country_task set status = '5' where id = '88fe0c336da640f788e2563c8958596a';
