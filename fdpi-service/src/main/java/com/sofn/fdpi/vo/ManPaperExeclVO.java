 ...
 */
@Data
@ExcelSheetInfo(sheetName = "经营利用许可证导入模板")
public class ManPaperExeclVO {
    @ExcelField(title = "序号")
    ...
    @ExcelImportCheck(isNull =  false,max=20,errMsg = "企业名称不能为空且长度不超过20位")
    ...
    @ExcelImportCheck(isNull =  false,max=10,errMsg = "法人代表不能为空且不长度超过10位")
    private String legal;

    @ExcelField(title = "经营方式")
    @ExcelImportCheck(isNull =  false,max = 10,errMsg = "经营方式不能为空且长度不超过10位")
    private String modeOperation;

    @ExcelField(title = "销售去向")
    @ExcelImportCheck(isNull =  false,max = 10,errMsg = "销售去向不能为空且长度不超过10位")
    ...
    public static ManPaperExeclVO form2Vo(ManPaperExeclFrom from) {
        ManPaperExeclVO vo = new ManPaperExeclVO();
