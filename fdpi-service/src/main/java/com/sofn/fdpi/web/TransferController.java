...
import com.sofn.fdpi.enums.TranferStatusEnum;
 ...
 */
@Slf4j
@Api(value = "物种转移相关接口", tags = "物种转移相关接口")
@RestController
...
public class TransferController {
    ...
    public Result<List<TransferVO>> listTransfer(
        ...
        return Result.ok(transferService.listTransferVO2(MapUtil.getParams(keys, vals), pageNo, pageSize));
    }
...
//        }
    ...
    @ApiOperation("查询统计按照区域")
    @PostMapping("listSpeciesChangeNew")
    @RequiresPermissions(value = {"fdpi:regionStatistic:index", "fdpi:regionStatistic:query"}, logical = Logical.OR)
    public Result<AreaVO> listSpeciesChangeNew(@RequestParam(required = true) @ApiParam("时间起") String startTime,
                                               @RequestParam(required = true) @ApiParam("时间止") String endTime,
                                               @RequestParam(required = true) @ApiParam("物种ID") String speciesId,
                                               @RequestParam(required = true) @ApiParam("查询类型，1，全部省，2全部市，3，全部县，4，自定义") String queryType,
                                               @RequestParam(required = false) @ApiParam("区域代码，逗号隔开") String regiones,
                                               @RequestParam(required = false) @ApiParam("排序类型，1：物种存量，2：物种变化量，3：标识申请数，4：标识使用数，5:经营证书，6：繁育证书") String order,
                                               @RequestParam(required = false) @ApiParam("排序标志，'ASC':升序,'DESC':降序") String orderSign,
                                               @RequestParam(value = "pageNo") Integer pageNo,
                                               @RequestParam(value = "pageSize") Integer pageSize

    ) {
        String[] keys = {"startTime", "endTime", "speciesId", "regiones", "order", "orderSign", "queryType"};
        String[] vals = {startTime + " 00:00:00", endTime + " 23:59:59", speciesId, regiones, order, orderSign, queryType};
        return Result.ok(transferService.listSpeciesCountVORegionesNew(MapUtil.getParams(keys, vals), pageNo, pageSize));
    }
    ...
    public void exportSpeciesChange(@RequestParam(required = true) @ApiParam("时间起") String startTime,
        ...
        Map map = new HashMap();
        map.put("startTime", startTime + " 23:59:59");
        map.put("endTime", endTime + " 23:59:59");
        map.put("speciesId", speciesId);
