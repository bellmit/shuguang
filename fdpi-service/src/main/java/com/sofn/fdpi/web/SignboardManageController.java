 ...
 **/
@Slf4j
@Api(value = "标识管理相关接口", tags = "标识管理相关接口")
@RestController
@RequestMapping(value = "/signboardManage")
public class SignboardManageController extends BaseController {
    ...
    @RequiresPermissions("fdpi:signboardInComp:active")
...
//        signboardService.activateBatch(idList, status);
    ...
    @PutMapping("/update")
    @RequiresPermissions("fdpi:signboardInComp:update")
    ...
    @GetMapping("/get")
    @RequiresPermissions(value = {"fdpi:signboardManage:view", "fdpi:compInformation:view", "fdpi:signboardInComp:view"}, logical = Logical.OR)
    ...
    public Result<SignboardVo> getByCode(
        ...
        if (StringUtils.isEmpty(code)) {
            throw new SofnException("编码不能为空");
        }
...
//            @ApiParam("完整标识编码(精确查找全国,后端自动去除其它查询条件)") @RequestParam(required = false) String completeCode,
        ...
        String[] keys = {"applyCode", "status", "code", "isPedigree", "speName", "speId", "compName",
                "province", "city", "district", "delFlag"};
        Object[] vals = {applyCode, status, code, isPedigree, speName, speId, compName, province, city, district, BoolUtils.N};
        return Result.ok(signboardService.listPage(MapUtil.getParams(keys, vals), pageNo, pageSize));
    }

    @ApiOperation(value = "分页查询获取总数量")
    @GetMapping("/listPageTotalCount")
    public Result<Long> listPageTotalCount(
            @ApiParam("省代码") @RequestParam(required = false) String province,
            @ApiParam("市代码") @RequestParam(required = false) String city,
            @ApiParam("区县代码") @RequestParam(required = false) String district,
            @ApiParam("标识编码(模糊查找,如果为16位,则后端转换为精确查找)") @RequestParam(required = false) String code,
//            @ApiParam("完整标识编码(精确查找全国,后端自动去除其它查询条件)") @RequestParam(required = false) String completeCode,
        ...
        String[] keys = {"applyCode", "status", "code", "isPedigree", "speName", "speId", "compName",
                "province", "city", "district", "delFlag"};
        Object[] vals = {applyCode, status, code, isPedigree, speName, speId, compName, province, city, district, BoolUtils.N};
        return Result.ok(signboardService.listPageTotalCount(MapUtil.getParams(keys, vals)));
    }

    @ApiOperation(value = "分页查询获取当页数据")
    @GetMapping("/listPageData")
    public Result<PageUtils<SignboardVo>> listPageData(
            @ApiParam("省代码") @RequestParam(required = false) String province,
            @ApiParam("市代码") @RequestParam(required = false) String city,
            @ApiParam("区县代码") @RequestParam(required = false) String district,
            @ApiParam("标识编码(模糊查找,如果为16位,则后端转换为精确查找)") @RequestParam(required = false) String code,
//            @ApiParam("完整标识编码(精确查找全国,后端自动去除其它查询条件)") @RequestParam(required = false) String completeCode,
        ...
        String[] keys = {"applyCode", "status", "code", "isPedigree", "speName", "speId", "compName",
                "province", "city", "district", "delFlag"};
        Object[] vals = {applyCode, status, code, isPedigree, speName, speId, compName, province, city, district, BoolUtils.N};
        return Result.ok(signboardService.listPageData(MapUtil.getParams(keys, vals), pageNo, pageSize));
    }
    ...
    @RequiresPermissions(value = {"fdpi:signboardApply:signboardList", "fdpi:signboardApprove:signboardList"}, logical = Logical.OR)
    public Result<List<SignboardApplyListVo>> getListInfo(
            ...
            @ApiParam("获取当前企业非转移中的标识(如果不传,后端则取当前用户单位ID)") @RequestParam(required = false) String queryType,
            @ApiParam("物种ID") @RequestParam(required = false) String speId) {
