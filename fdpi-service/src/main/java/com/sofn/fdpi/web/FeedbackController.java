...
import com.sofn.fdpi.vo.FeedbackVo;
...
//    @RequiresPermissions(value = {"fdpi:feedbackByGZ:create","fdpi:feedbackByZF:create"},logical = Logical.OR)
    ...
    public Result<Feedback> saveFeedback(
            @RequestBody @ApiParam(name = "反馈信息对象", value = "传入json格式", required = true)
            @Validated FeedbackInfoVo feedbackInfoVo, BindingResult result) {
        if (result.hasErrors()) {
...
//        feedbackInfoVo.setFfFrom("1");
    ...
    @RequiresPermissions("fdpi:feedbackByGZ:advice")
    @SofnLog("反馈")
