...
@Slf4j
@Api(value = "物种", tags = "物种相关接口")
@RestController
@RequestMapping("/spe")

public class SpeController extends BaseController {
    @Autowired
    ...
    public Result<PageUtils<Spe>> getSpeByPage(@ApiParam(name = "cites", value = "CITES级别，1级：1,2级：2,3级：3", required = false) @RequestParam(value = "cites", required = false) String cites,
        ...
        String[] keys = {"speName", "identify", "pedigree", "proLevel", "cites"};
        Object[] vals = {speName, identify, pedigree, proLevel, cites};
        return Result.ok(speService.listSpeByPage(MapUtil.getParams(keys, vals), pageNo, pageSize));
    }
...
//    getSpeciesName
    ...
    public Result<String> importSite(@RequestParam @ApiParam("文件导入地址") MultipartFile multipartFile) {
        try {
            ...
            speService.add(importList);
        } catch (Exception e) {
            e.printStackTrace();
            throw new SofnException(e.getMessage());
        }
//        speService.importData(multipartFile,speService);
        return Result.ok();
    }
    ...
    @SofnLog("导出物种信息")
    @ApiOperation(value = "导出物种信息", produces = "application/octet-stream")
    @GetMapping(value = "/export", produces = "application/octet-stream")
    public void export(HttpServletResponse hsr, @ApiParam(name = "cites", value = "CITES级别，1级：1,2级：2,3级：3", required = false) @RequestParam(value = "cites", required = false) String cites,
                       @ApiParam(name = "proLevel", value = "中国保护水平写死 一级:1 ，2级：2 ，特殊管理要求：3", required = false) @RequestParam(value = "proLevel", required = false) String proLevel,
                       @ApiParam(name = "pedigree", value = "是否进行谱系写死，否：0是：1", required = false) @RequestParam(value = "pedigree", required = false) String pedigree,
                       @ApiParam(name = "speName", value = "物种名", required = false) @RequestParam(value = "speName", required = false) String speName,
                       @ApiParam(name = "identify", value = "是否使用标识写死，0:不使用,1 :全程使用,2:销售使用", required = false) @RequestParam(value = "identify", required = false) String identify) {
        String[] keys = {"speName", "identify", "pedigree", "proLevel", "cites"};
        Object[] vals = {speName, identify, pedigree, proLevel, cites};
        speService.export(MapUtil.getParams(keys, vals), hsr);
    }

    @ApiOperation(value = "重新设置物种缓存")
    @GetMapping(value = "/setCache")
    public void setCache() {
        speService.saveNameredis();
    }
}
