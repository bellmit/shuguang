...
@Slf4j
@Api(value = "机构相关接口（证书绑定直属机构/证书年审直属机构）", tags = "机构相关接口（证书绑定直属机构/证书年审直属机构）")
@RestController
...
public class TbDepartmentController extends BaseController {
    ...
    @ApiOperation(value = "证书绑定直属机构->《获取直属部门对象》")
    ...
    @ApiOperation(value = "证书年审直属机构-》《分页列表》")
    ...
    @ApiOperation(value = "是否直属机构")
    @GetMapping("/isDirectlyDept")
    public Result<Boolean> isDirectlyDept() {
        return Result.ok(tbDepartmentService.isDirectlyDept());
    }
