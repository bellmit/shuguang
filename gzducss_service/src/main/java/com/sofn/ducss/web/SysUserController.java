...
import com.sofn.ducss.service.SysRoleService;
import com.sofn.ducss.service.SysSubsystemService;
import com.sofn.ducss.service.SysUserRolePermissionService;
...
public class SysUserController extends BaseController {
    ...
    public Result<String> create(@Validated @RequestBody SysUserForm sysUserForm, BindingResult result) {
        ...
        String userId= userService.saveSysUser(sysUser);
        LogUtil.addLog(LogEnum.LOG_TYPE_USER_ADD.getCode(), "添加-" + UserUtil.getLoginUserName() + "-<用戶新增>");
        //将Id放入ok
...
//    @RequiresPermissions("sys:user:update")
    ...
    public Result<String> update(@Validated @RequestBody SysUserForm sysUserForm, BindingResult result) {
        ...
        LogUtil.addLog(LogEnum.LOG_TYPE_USER_EDIT.getCode(), "编辑-" + UserUtil.getLoginUserName() + "-<用戶编辑>");
        return Result.ok();
    }
    ...
    public Result<String> deleteBatchByIds(
        ...
        LogUtil.addLog(LogEnum.LOG_TYPE_USER_DELETE.getCode(), "删除-" +UserUtil.getLoginUserName()+ "-<用戶删除>");
        return Result.ok();
    }
    ...
    public Result<SysUserForm> getLoginUser() {
        ...
        if (user != null) {
           ...
           // throw new TokenException("Unauthorized", 401);
        }
    }
    ...
    public Result<String> importSysUser(@ApiParam(value = "Excel文件") @RequestPart(value = "file") MultipartFile file) {
        try {
            ...
            if (importSysUserVoList.size() == 0){
                return Result.ok("导入数据为空");
            }
