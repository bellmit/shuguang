...
public class CountryTaskController {
    ...
    public Result<PageUtils<CountryTask>> getCountryTaskList(@RequestParam(value = "pageNo") @ApiParam(value = "起始行数") Integer pageNo
        ...
        PageUtils<CountryTask> countryTasks = countryTaskService.getTaskByPage(pageNo, pageSize, countyId, year);
    ...
    public Result<PageUtils<CountryTask>> getMinistryTaskList(@RequestParam(value = "pageNo") @ApiParam(value = "起始行数") Integer pageNo
        ...
        PageUtils<CountryTask> countryTasks = countryTaskService.getMinistryTaskByPage(pageNo, pageSize, year);
    ...
    @SofnLog("部级补发任务")
    @ApiOperation(value = "部级补发任务")
    @GetMapping("/supplyTask")
    public Result<String> supplyTask(String year, HttpServletRequest request) {
        String userId = UserUtil.getLoginUserId(request.getHeader("Authorization"));
        if (StringUtils.isBlank(userId)) {
            throw new SofnException("当前登录用户异常");
        }
        return Result.ok(countryTaskService.supplyTask(year));
    }
