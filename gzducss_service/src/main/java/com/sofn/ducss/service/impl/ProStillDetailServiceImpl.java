...
public class ProStillDetailServiceImpl extends ServiceImpl<ProStillDetailMapper, ProStillDetail> implements ProStillDetailService {
    ...
    public List<ProStillDetail> getProStillDetail(String proStillId) {
        ...
        if (list2.size() > 0) {
            ...
            Map<String, ProStillDetail> detailMap = list2.stream().collect(Collectors.toMap(ProStillDetail::getStrawType, Function.identity(), (key1, key2) -> key2));
            ...
            while (iterator.hasNext()) {
                ...
                for (ProStillDetail proStillDetail : list2) {
                    String type = proStillDetail.getStrawType();
                    if (key.equals(type)) {
                        proStillDetail.setStrawName(sysDict.getDictValue());
            ...
            if (detailMap.size() > 0) {
                ...
                for (SysDictionary sysDict : otherDict) {
                    for (Map.Entry<String, ProStillDetail> entry : detailMap.entrySet()) {
                        if (entry.getKey().equals(sysDict.getDictKey())) {
                            ProStillDetail detail = entry.getValue();
                            if (detail.getGrainYield().compareTo(BigDecimal.ZERO) > 0
                                    || detail.getReturnArea().compareTo(BigDecimal.ZERO) > 0
                                    || detail.getSeedArea().compareTo(BigDecimal.ZERO) > 0
                                    || detail.getExportYield().compareTo(BigDecimal.ZERO) > 0) {
...
/*            if (CollectionUtils.isNotEmpty(list3)) {
    ...
    public String addOrUpdateProStill(ProStillVo proStillVo, String userId) {
        ...
        if (StringUtils.isEmpty(userAreaByLogUser)) {
...
//            }
    ...
    public List<YieldAndReturnExportExcel> getProStillExportExcelById(String proStillId) {
        ...
        String countyName = sysApi.getRegionNamesByCodes(proStill.getAreaId());
        //导出排序
        List<SysDictionary> list = sysDictionaryMapper.getAllSysDictionaries(Constants.DictionaryType.STRAW_TYPE);
    ...
    @Override
    public List<YieldAndReturnExportExcel> getExportExcelByAredIdAndYear(String areaId, String year) {
        String areaIds = "";
        ...
        List<ProStillDetail> detailList = proStillDetailMapper.getProStillDetailListByAreaId(areaIds,
