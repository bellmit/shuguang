...
public class SysUserServiceImpl extends ServiceImpl<SysUserMapper, SysUser> implements SysUserService {
    ...
    public Auth execuLogin(LoginVo loginVo, boolean isApp, String type) {
        //执行登录前先校验登录失败次数,判定是否限制状态
        Boolean isCanLogin = SysCacheUtils.getFailCountByUserName(loginVo.getUsername());
        if (!isCanLogin) {
            List<SysUser> sysUsers = this.baseMapper.selectList(new QueryWrapper<SysUser>().eq("username", loginVo.getUsername()).eq("del_flag", "N"));
            if (sysUsers.size() != 1) {
                //username只能查出一个用户,为空或多个都是异常情况
                throw new SofnException("用户数据异常！");
            }
            //修改用户账号状态(0:限制,1:正常,2:禁用,3:注销)
            throw new SofnException("当前用户异常(用户状态：" + SysManageEnum.getEnumDes("STATUS", "0") + ")");
        }
    ...
    public void updatePassword(String id, String oldPassword, String newPassword) {
        ...
        sysUser.setSalt(salt);
        sysUser.preUpdate();
        //在修改时把是否已修改密码状态修改为 "1":已修改
        Integer updated = SysConstant.UPDATED_1;
        sysUser.setUpdated(updated);
    ...
    public void setData(List<SysUser> sysUserList, List<SysUserRole> sysUserRoles, List<SysUserGroup> sysUserGroups, Map<String, List> data, List<ImportSysUserVo> userList) {
        if (!CollectionUtils.isEmpty(userList) && !CollectionUtils.isEmpty(data)) {
            ...
            userList.forEach(userInfo -> {
                ...
                String salt = IdUtil.getUUId();
                String pdMd5 = DigestUtils.md5DigestAsHex(userInfo.getInitPassword().trim().getBytes());
                String password = ShiroUtils.sha256(pdMd5, salt);
                sysUser.setPassword(password);
                sysUser.setSalt(salt);
                sysUser.setNickname(userInfo.getNickname());
                sysUser.setUsername(userInfo.getUserName().trim());
                String email = userInfo.getEmail();
                sysUser.setEmail(email);
                sysUser.setUpdated(1);
                sysUser.setRemark(UserUtil.getLoginUserId() + "批量导入用户");
                sysUser.preInsert();
                sysUser.setCreateTime(new Date());
                sysUser.setInitPassword(Constants.DEFAULT_PASSWORD); // 设为默认值
                // 获取机构ID
...
/*                String roleGroupNames = userInfo.getGroupNames();
    ...
    public List<ImportSysUserVo> getExportUserInfo(Map<String, String> params) {
        if (!CollectionUtils.isEmpty(params)) {
            List<ImportSysUserVo> list = sysUserMapper.getExportUserInfo(params);
            if(list != null && list.size() > 0 ){
                list.forEach(item->{
                    if (StringUtils.isNotEmpty(item.getRoleNames())){
                        item.setRoleNames(item.getRoleNames().substring(0,item.getRoleNames().length()-1));
                    }
                });
            }
            return list;
...
//        return PageUtils.getPageUtils(pageInfo);
    ...
    @Override
    public List<SysUser> findByUserIds(Map<String, Object> map) {
        return sysUserMapper.getSysUserByCondition(map);
    }

    public static void main(String[] args) {
/*        String saltPassword = ShiroUtils.sha256("123456", "23024d961b7e4ed2ae93b6773f1359f5");
        System.out.println(saltPassword);*/
        //0acc08a2c2c00fa6839b781ffb93868e6f88a02930ed725734203721e9726e08
        // b41cb62ec6767f2e41f9df7a2d161515
        String pdMd5 = DigestUtils.md5DigestAsHex("123456".getBytes());
        System.out.println(pdMd5);
        String password = ShiroUtils.sha256(pdMd5, "30c235e34a7b4908abdf7a014051bb6d");
        System.out.println(password);


        String saltPassword = ShiroUtils.sha256("e10adc3949ba59abbe56e057f20f883e", "30c235e34a7b4908abdf7a014051bb6d");
        System.out.println(saltPassword);
    }
