...
public class ProvinceReportServiceImpl implements ProvinceReportService {
    ...
    public Map<String, Object> getProvinceReport(String provinceId, String year) {
        ...
        //获取省名称
        String provinceName = sysApi.getSysRegionName(provinceId);
        ...
        if (list.isEmpty() == false) {
            StrawProduceUtilizeResVo2 provinceProduce = list.get(0);
            ReturnLeaveSumVo returnLeaveSumVo = new ReturnLeaveSumVo();
            if (!returnLeaveSumVoList.isEmpty()) {
            ...
            if (listLast.isEmpty() == false) {
                ReturnLeaveSumVo returnLeaveSumVoLast = new ReturnLeaveSumVo();
                if (!returnLeaveSumVoListLast.isEmpty()) {
                    returnLeaveSumVoLast = returnLeaveSumVoListLast.get(0);
                }
                BigDecimal lastComprehensice = listLast.get(0).getComprehensive();
                //比较数据
                ...
                BigDecimal lastComprehensiceIndex = listLast.get(0).getIndustrializationIndex();
                //产业化利用能力指数比较数据
                ...
                BigDecimal lastMainMartketPerc = listLast.get(0).getCollectResource().compareTo(new BigDecimal(0)) == 0 ? new BigDecimal(0) : lastMainMarket.divide(listLast.get(0).getCollectResource(), 2, RoundingMode.HALF_UP);
        ...
        if (proListTemp.isEmpty() == false) {
            ...
            for (int i = 0; i < proList.size(); i++) {
                mapData.put(proList.get(i).getAreaName(), BigDecimalUtil.getTenThousand(proList.get(i).getTheoryResource()).setScale(2, RoundingMode.HALF_UP).doubleValue());
            }
            datas.put("", mapData);
            //柱状图转为base64
            ...
            for (int i = 0; i < proList.size(); i++) {
                ...
                ComprehensiveMap.put(proList.get(i).getAreaName(), proList.get(i).getComprehensive().setScale(2, RoundingMode.HALF_UP).doubleValue());
            }
            ...
            for (int i = 0; i < proList.size(); i++) {
                ComprehensiveIndexMap.put(proList.get(i).getAreaName(), proList.get(i).getIndustrializationIndex().doubleValue());
            }
        ...
        if (comRatioListTemp.isEmpty() == false) {
            //去除合计部分
            ...
            Map<String, Double> comprehensiveMap = new LinkedHashMap<>();
            for (int i = 0; i < comRatioList.size(); i++) {
                comprehensiveMap.put(comRatioList.get(i).getStrawName(), comRatioList.get(i).getComprehensiveRate().setScale(2, RoundingMode.HALF_UP).doubleValue());
            }
