...
import com.sofn.ducss.vo.CollectFlowVo;
...
public class CollectFlowServiceImpl extends ServiceImpl<CollectFlowMapper, CollectFlow> implements CollectFlowService {
    ...
    @Override
    public PageUtils<CollectFlow> getCollectFlowByPage(Integer pageNo, Integer pageSize, String year, String areaId) {
    ...
    public CollectFlowWithTotalVo getWaitForExamineDataForCity(String year, String status, String cityId) {
        ...
        List<SysRegionTreeVo> sysRegionTreeVoList = sysApi.getListByParentId(cityId, Constants.APPID, "");

        //获取所有下级的区划ID
        String childrenIds = SysRegionUtil.getChildrenRegionIds(cityId);
        //获取已上报的数量
        ...
        for (SysRegionTreeVo sysRegionTreeVo : sysRegionTreeVoList) {
            ...
            String regionName = "";
            String result = sysApi.getSysRegionName(areaId);
            if (null != result) {
                regionName = result;
            }
    ...
    public CollectFlowWithTotalVo getWaitForExamineDataForProvince(String year, String status, String provinceId) {
        ...
        List<SysRegionTreeVo> sysRegionTreeVoList = sysApi.getListByParentId(provinceId, Constants.APPID, "");

        //获取所有下级的区划ID
        String childrenIds = SysRegionUtil.getChildrenRegionIds(provinceId);
        //获取已上报的数量
        Integer regionReportNum = collectFlowMapper.countReported(year, Constants.ExamineState.REPORTED.toString(), childrenIds);
    ...
    public synchronized String updateStatusForCity(UpdateStatusVo reqVO, String userRegionId, String level, String userId, String userName) {
        ...
        if (reqVO.getStatus().equals(Constants.ExamineState.REPORTED)) {
            return statusReported(reqVO, userRegionId);
        }
        // 阅读
...
//            countryTaskService.insertOrUpdateSuperTaskNum(year, userRegionId);
    ...
    private String statusPass(UpdateStatusVo reqVO, CollectFlow ducCollectFlow, String userRegionId, String level, String userId, String userName) {
        if (level.equals(RegionLevel.CITY.getCode())) {
            ...
            List<SysRegionTreeVo> data = sysApi.getParentNode(areaId);
            //获取市Id
            SysRegionTreeVo city = data.get(0);
        ...
        if (ducCollectFlow.getStatus().equals(Constants.ExamineState.REPORTED)
            ...
            String areaName = SysRegionUtil.getFullRegionNameByLastCode(ducCollectFlow.getAreaId());
...
//        }
    ...
    // 审核上报
    private String statusReported(UpdateStatusVo reqVO, String userRegionId) {
        ...
        Boolean flag = false;
        List<SysRegionTreeVo> sysRegionTreeVoList = sysApi.getListByParentId(userRegionId, Constants.APPID, "");
        for (SysRegionTreeVo sysRegionTreeVo : sysRegionTreeVoList) {
            String areaId = sysRegionTreeVo.getRegionCode();
            String regionName = "";
            String result = sysApi.getSysRegionName(areaId);
        ...
        //获取所有下级的区划ID
        String childrenIds = SysRegionUtil.getChildrenRegionIds(userRegionId);
        ArrayList<String> status = new ArrayList<>();
        status.add(AuditStatusEnum.PASSED.getCode());
        // 查询秸秆产生量数据
        List<StrawProduce> proList = strawProduceMapper.sumStrawProduce2(userRegionId, ducCollectFlow.getYear(), status, IdUtil.getIdsByStr(childrenIds));
        if (!proList.isEmpty()) {
        ...
        // 查询秸秆利用量数据
        List<StrawUtilizeSum> utilizeList = strawUtilizeSumMapper.sumStrawUtilizeSum(userRegionId, ducCollectFlow.getYear(), status, IdUtil.getIdsByStr(childrenIds));
        ...
        StrawUtilizeSum sum = strawUtilizeSumMapper.selectThanCountrySum(reqVO.getAreaId(), reqVO.getYear(), status, IdUtil.getIdsByStr(childrenIds));
        if (sum != null) {
...
//            ducCountryTaskService.insertOrUpdateSuperTaskNum(ducCollectFlow.getYear(), ducArea.getParentId());
    ...
    private void addReportAndWithrawnMessage(CollectFlow ducCollectFlow, String status) {
        ...
        //获取县名
        String regionName = "";
        String result = sysApi.getSysRegionName(ducCollectFlow.getAreaId());
        if (null != result) {
            regionName = result;
        }
        //消息内容
...
//    private StrawProduceMapper strawProduceMapper;
    ...
    public List<StrawProduceResVo> findStrawProduceData(String areaId, String year) throws JsonProcessingException, IllegalAccessException, InvocationTargetException {
        ...
        if (!result.equals("00")) {
            ...
            //获取所有下级的区划ID
            String childrenIds = SysRegionUtil.getChildrenRegionIds(areaId);
    ...
    public List<StrawUtilizeSum> findStrawUtilzeData(String areaId, String year) throws JsonProcessingException {
        ...
        List<SysDict> dictList = sysApi.getDictListByType(Constants.DictionaryType.STRAW_TYPE);
