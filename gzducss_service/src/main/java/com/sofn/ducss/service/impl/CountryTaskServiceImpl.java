...
import com.sofn.ducss.util.SumUtil;
...
public class CountryTaskServiceImpl extends ServiceImpl<CountryTaskMapper, CountryTask> implements CountryTaskService {
    ...
    @Override
    public PageUtils<CountryTask> getTaskByPage(Integer pageNo, Integer pageSize, String countyId, String year) {
    ...
    @Override
    public PageUtils<CountryTask> getMinistryTaskByPage(Integer pageNo, Integer pageSize, String year) {
        PageHelper.offsetPage(pageNo, pageSize);
        Map<String, Object> params = Maps.newHashMap();
        params.put("year", year);
    ...
    public String addAndIssueMinistryTask(CountryTask task) {
        ...
        if (tasks.size() <= 0) {
            ...
            Map<String, Object> regionParams = Maps.newHashMap();
            regionParams.put("parentId", "440981");
            regionParams.put("versionCode", sysRegionService.getMaxVersionCodeByYear(Integer.valueOf(task.getYear())));
            List<SysRegion> regionInfo = sysRegionMapper.getSysRegionByContion(regionParams);

            // List<SysRegion> regionInfo = syncSysRegionService.getAreaIdByParentId("440981");
            if (CollectionUtils.isEmpty(regionInfo)) {
                throw new SofnException("未获取到区县信息");
            }

            for (SysRegion item : regionInfo) {
    ...
    @Override
    public String supplyTask(String year) {
        // 查询当年任务
        Map<String, Object> ministryQueryMap = Maps.newHashMap();
        ministryQueryMap.put("year", year);
        ministryQueryMap.put("taskLevel", RegionLevel.MINISTRY.getCode());
        List<CountryTask> ministryTask = countryTaskMapper.getCountryTaskByCondition(ministryQueryMap);
        if (ministryTask.size() > 0) {
            // 当年任务存在
            CountryTask countryTask = ministryTask.get(0);
            if (Constants.ExamineState.PUBLISH.equals(countryTask.getStatus())) {
                return "任务已公布，无法补发任务！";
            }
            // 查询已下发的县
            Map<String, Object> countyQueryMap = Maps.newHashMap();
            countyQueryMap.put("year", year);
            countyQueryMap.put("taskLevel", RegionLevel.COUNTY.getCode());
            // countyQueryMap.put("statusList", Lists.newArrayList(Constants.ExamineState.ISSUE.toString(), Constants.ExamineState.PUBLISH.toString()));
            List<CountryTask> countyTask = countryTaskMapper.getCountryTaskByCondition(countyQueryMap);
            List<String> countyIds = countyTask.stream().map(CountryTask::getAreaId).collect(Collectors.toList());
            // 查询所有的县级id
            List<AreaRegionCode> allCountyIds = SysRegionUtil.getAllCountyIds();
            // 需要补发的县 = 所有县 - 已下发的县
            List<AreaRegionCode> supplyCountyIds = allCountyIds.stream().filter(e -> !countyIds.contains(e.getCountyId())).collect(Collectors.toList());
            if (CollectionUtils.isNotEmpty(supplyCountyIds)) {
                // 补发任务
                List<CountryTask> supplyTaskList = new ArrayList<>();
                for (AreaRegionCode supplyCounty : supplyCountyIds) {
                    CountryTask supplyTask = new CountryTask();
                    supplyTask.setId(IdUtil.getUUId());
                    supplyTask.setYear(year);
                    supplyTask.setProvinceId(supplyCounty.getProvinceId());
                    supplyTask.setCityId(supplyCounty.getCityId());
                    supplyTask.setAreaId(supplyCounty.getCountyId());
                    supplyTask.setExpectNum(countryTask.getExpectNum());
                    supplyTask.setStatus(Constants.ExamineState.SAVE);
                    supplyTask.setIsReport((byte) 0);
                    supplyTask.setTaskLevel(RegionLevel.COUNTY.getCode());
                    supplyTask.setCreateUserId(countryTask.getCreateUserId());
                    supplyTask.setCreateUserName(countryTask.getCreateUserName());
                    supplyTask.setCreateDate(new Date());
                    supplyTaskList.add(supplyTask);
                }
                if (CollectionUtils.isNotEmpty(supplyTaskList)) {
                    saveBatch(supplyTaskList);
                    return "补发任务成功！";
                }
            }
            return "无任务补发！";
    ...
    public Result examineCountryTask(CountryTask task, String userName, String userId) {
        ...
        if (tasks.size() == 1) {
            ...
            if (oldTask.getStatus() == status) {
                ...
                String areaName = SysRegionUtil.getFullRegionNameByLastCode(areaId);
    ...
    private void addReportMessage(CountryTask task, String status) {
        ...
        //获取县名
        String regionName = "";
        String result = sysApi.getSysRegionName(task.getAreaId());
        if (null != result) {
            regionName = result;
        }
        //消息内容
...
//  	}
    ...
    private List<StrawUtilizeSum> getProList(String areaId, String year) {
        ...
        List<SysDict> list2 = sysApi.getDictListByType(Constants.DictionaryType.STRAW_TYPE);
...
//    }
    ...
    public TapVo getTapVo(String year, String regioncode) {
        ...
        List<SysRegionTreeVo> sysRegionTreeVoList = sysApi.getListByParentId(regioncode, Constants.APPID, "");
