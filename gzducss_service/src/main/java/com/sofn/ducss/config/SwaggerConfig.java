...
public class SwaggerConfig {
    ...
    private List<ApiKey> securitySchemes() {
        return  Lists.newArrayList(new ApiKey(JWTToken.TOKEN,JWTToken.TOKEN,"header"));
    }

    /**
     * 在Swagger2的securityContexts中通过正则表达式，设置需要使用参数的接口（口），
     * 如下列代码所示，通过PathSelectors.regex("^(?!auth).*$")，
     * 所有包含"auth"的接口不需要使用securitySchemes。即或者说，是去除掉不需要使用参数的接不需要使用上文中设置的名为“Authorization”，
     * type为“header”的参数。
     * @return
     */
