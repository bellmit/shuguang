-- 处理 中国河北省辛集市市级丢失数据
-- 辛集 : 130181  ；辛集虚拟县 : 	139181

-- 18年collectFlow数据没有县级只有市级
-- straw_utilize_sum_total 丢失18年县级数据
-- straw_utilize_sum 丢失18年数据

-- 1.迁移产生量与直接还田量填报
UPDATE pro_still SET area_id= '139181',city_id = '130181'  WHERE  ID IN (SELECT ID
FROM pro_still WHERE area_id = '139181' );
-- 2.农户分散利用量填报
UPDATE disperse_utilize SET area_id = '139181',city_id = '130181'  WHERE ID IN (SELECT "id" FROM disperse_utilize WHERE area_id = '139181' );
-- 3.迁移市场主体利用量填报数据
UPDATE straw_utilize SET area_id = '139181',city_id = '130181' WHERE ID IN (SELECT "id" FROM straw_utilize  WHERE area_id = '139181');

UPDATE pro_still SET city_id = '130181' ,area_id = '139181' where area_id = '130181';
UPDATE disperse_utilize SET area_id = '139181' where area_id = '130181';
UPDATE straw_utilize SET city_id = '130181' ,area_id = '139181' where area_id = '130181';


update collect_flow set city_id = '130181',level = '3' where area_id = '139181';
update collect_flow set city_id = '130181',level = '4' where area_id = '130181';

update straw_utilize_sum_total set province_id = '130000',city_id = '130181' where area_id = '139181';
update straw_utilize_sum_total set province_id = '130000',city_id = '130181' where area_id = '130181';

update straw_utilize_sum set city_id = '130181' where area_id = '139181';
update straw_utilize_sum set city_id = '130181' where area_id = '130181';
...
INSERT INTO collect_flow (
	...
	buy_other_num,
	export_num
) SELECT year||province_id||city_id||'139181' as "id"
,
YEAR,
province_id,
'130181',
...
INSERT INTO straw_utilize_sum_total (
	...
	year||province_id||city_id||'139181' as "id",
	YEAR,
	province_id,
	'130181',
	'139181',
...
id,
	...
	export_yield_total,
	grain_yield
) SELECT year||'139181'||straw_type as "id",YEAR
,
province_id,
'130181',
'139181',
...
FROM
	...
	WHERE straw_type=dict_key and year = '2018' and area_id = '139181');
...
SELECT year||'139181'||straw_type as "id"
,
YEAR,
'139181',
straw_type,
...
FROM
	...
	AND A.YEAR IN('2018') AND a.area_id = '139181'
	) C;
...
WHERE
	D.B_ID = A.B_ID AND A.YEAR IN('2018') AND a.area_id = '139181';
...
RETURN_RESOURCE = (
	...
	WHERE A.YEAR IN('2018') AND a.area_id = '139181';

update data_analysis_area a
set COLLECT_RESOURCE = ceil( ceil(GRAIN_YIELD * GRASS_VALLEY_RATIO * 100)/100 * COLLECTION_RATIO *100)/100
from (SELECT a.year,a.area_id from ( SELECT year,area_id,sum(collect_resource) as "collect_resource" FROM data_analysis_area WHERE year in('2018') AND area_id = '139181' GROUP BY year,area_id) a left join (SELECT id,year,area_id,collect_num,create_date from collect_flow WHERE year in('2018') and area_id = '139181') b on b.area_id = a.area_id
where a.collect_resource != b.collect_num and a.year = b.year) b
where a.year = b.year and a.area_id = b.area_id AND A.YEAR IN('2018') and A.area_id = '139181';

update data_analysis_area a
set RETURN_RESOURCE = round((collect_resource * return_ratio)/100,10)
from (SELECT a.year,a.area_id from ( SELECT year,area_id,round(sum((collect_resource * return_ratio)/100),10) as "re_num" FROM data_analysis_area WHERE year in('2018') and area_id = '139181' GROUP BY year,area_id) a left join (SELECT id,year,area_id,direct_return_num,create_date from collect_flow WHERE year in('2018') and area_id = '139181') b on b.area_id = a.area_id
      where a.re_num != b.direct_return_num and a.year = b.year and a.area_id = b.area_id) b
where a.year = b.year and a.area_id = b.area_id AND A.YEAR IN('2018') and A.area_id = '139181';

update data_analysis_area set RETURN_PERS = return_ratio / 100 where YEAR IN('2018') and area_id = '139181';
...
WHERE
	A.B_ID = D.B_ID AND A.YEAR IN('2018') and A.area_id = '139181';
...
FROM
    ...
    WHERE
	A.B_ID = F.B_ID AND F.YEAR IN('2018') AND F.AREA_ID = '139181';
...
FROM
    ...
    where t2.year IN('2018') and t2.area_id = '139181'
	GROUP BY
		B_ID
	) D
WHERE
...
SET REUSE = FERTILISINGD + FORAGED + FUELD + BASED + MATERIALD WHERE YEAR IN('2018') AND AREA_ID = '139181';
...
FROM
	...
	FROM
		DATA_ANALYSIS_AREA
	where
	    year in ('2018') and AREA_ID = '139181'
	GROUP BY
		AREA_ID,
	YEAR
	) B
WHERE
	A.AREA_ID = B.AREA_ID
	AND A.YEAR = B.YEAR AND A.YEAR IN('2018');
...
FROM
	DATA_ANALYSIS_AREA
WHERE YEAR IN('2018','2019') and area_id = '139181'
GROUP BY
...
CASE
	...
	END WHERE YEAR IN('2018','2019') and city_id = '130181';
...
FROM
	...
	FROM
		data_analysis_city
	WHERE
	    year in ('2018','2019') and city_id = '130181'
	GROUP BY
		city_ID,
	YEAR
	) B
...
from (
	SELECT * from straw_utilize_sum where year in ('2018','2019') and area_id = '139181'
) b
where
	a.area_id = b.area_id and a.year = b.year and a.straw_type = b.straw_type and a.area_id = '139181';
...
--return_pers = , -- 单作物的直接还田率
from (
	SELECT * from straw_utilize_sum where year in ('2018','2019') and city_id = area_id and province_id != city_id and area_id = '130181'
) b
where
	a.year = b.year and a.city_id = b.city_id and a.straw_type = b.straw_type;

delete from data_analysis_area where area_id = '130181';

update data_analysis_area set area_name = '/河北省/辛集市/辛集市虚拟县' where area_id = '139181';
update data_analysis_city set area_name = '/河北省/辛集市' where city_id = '130181';